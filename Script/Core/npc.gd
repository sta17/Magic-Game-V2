...
signal npc_died(drop_position: Vector3)
...
@onready var _mesh:			MeshInstance3D	= $MeshInstance3D/MeshInstance3D3
...
func _do_chase(delta: float) -> void:
	...
	if dist > detection_range * 1.6:
		state = State.PATROL; return
	movement_component._move_toward(_player.global_position, move_speed, delta)
